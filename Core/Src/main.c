...
uint8_t Tik_045 = 0; // тик для медленной индикации, взводится каждые 450 мС , в основном цикле...
uint8_t Key_Read = 0; // тик для медленносчитывания клавиатуры , в основном цикле...
uint8_t Ena_ADC_Read = 0; //признак запуска измерения ADC
...
int main(void)
{
  ...
  while (1)
  {
//    if((Ena_ADC_Read)&&(GetSysTick(0)>8))
//    {
//      Ena_ADC_Read = 0;
...
//        ModeFuncTmp();
      ...
      if(ModeUSB) // устанавливаем когда пишем или читаем по USB SDCard
      {
        ...
        if(Tik_045) //  каждые .45 сек
        {
          ...
          {
          case 3:
            // вывод картинки о том что занято USB
            sprintf((void*)Str,"pic 144,94,%dяяя",(g_IndexMeas%3)+53);
            //sprintf((void*)Str,"pic 144,94,53яяя");
            NEX_Transmit((void*)Str);//
            MemMsgModeUSB = 1; // признак работы USB для индикации доп строчки
            ModeUSB=2;
            break;
          case 2:
