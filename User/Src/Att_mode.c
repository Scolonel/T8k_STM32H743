...
void ModeFuncTmp(void)
{
  ...
  if((g_ErrFW_LCD || g_OldFW_LCD) && TimerDraw)
  {
    ...
    if(g_OldFW_LCD)
    {
      sprintf( StrL,"xstr 10,80,460,40,2,RED,WHITE,1,1,1,\"%s\"яяя","ОБНОВИТЕ ПО LCD "); //  сообщение об ошибке FW LCD
      NEX_Transmit((void*)StrL);//
    }
    TimerDraw = 0;
  }
...
void ModeDrawMeasure(void) // режим отображения графического
{
  ...
  if(g_FirstScr) // заполнение незменяемых полей
  {
    ...
    if(UserSet.ChnMod == 0)
    {
      sprintf(Str,"t34.pco=65535яяя");// LW_nm = 1610
      NEX_Transmit((void*)Str);//
    }
    g_EnaQuickReDraw =1;
    ModeReDrawLCD = 1;
  ...
  if ((PRESS(BTN_OK))&&(getStateButtons(BTN_OK)==UP_SHORT_PRESSED)) // возврат в режим просмотра
  {
    ...
    {
      UserSet.ChnMod = 0;
      CmdInitPage(3);// посылка команды переключения окна на table
              sprintf(Str,"t34.pco=65535яяя");// LW_nm = 1610
        NEX_Transmit((void*)Str);//
...
void ModeKBComments(void) // режим отображения клавиатуры редактора Comments
{
  ...
  void ModeSaverFILE(void) // режим сохранения FILES
  {
    ...
    case 2: //Имя волокна
      ...
      break; //Имя волокна
    case 3: //№ волокна
      //  долго кнопка ОК сбрасываем счетчик волокон
    ...
    if(g_FirstScr)
    {
      ...
      NEX_Transmit((void*)Str);// 
      sprintf(Str,"t6.txt=\"%s\"яяя", MsgMass[32][CurrLang]); // № Волокна
      NEX_Transmit((void*)Str);// 
      sprintf(Str,"t8.txt=\"%s\"яяя", MsgMass[37][CurrLang]); //Комментарий
      NEX_Transmit((void*)Str);// 
      //      sprintf(Str,"t8.txt=\"%s\"яяя", MsgMass[34][CurrLang]); //ОК
      //      NEX_Transmit((void*)Str);// 
    ...
    if(g_NeedScr)
    {
      ...
      // четвертая сторка - номер волокна
      //sprintf(Str,"t7.txt=\"%s\"яяя", UserSet.Comments );//
      //NEX_Transmit((void*)Str);// 
      sprintf(Str,"t7.txt=\"%d\"яяя", UserSet.FiberID );//
      NEX_Transmit((void*)Str);// 
      // пятая строка Comments
      sprintf(Str,"t10.txt=\"%s\"яяя", (UserSet.Comments));//
      NEX_Transmit((void*)Str);// 
...
//    }
  ...
  if (g_NeedScr)
  {
    ...
    if(FR_Status == FR_OK)
    {
      ...
      i++;
      i++;// Зачемто съедаем пробел поставленнй в файле перед именем Кабеля
      StrR[len-2]=0;
